...
`Ir.Integer_interpreter` does not change the `#exe` boundary. It accepts an already checked in-memory IR graph and preflights every instruction, including unreachable blocks, before execution. Its allowlist covers internal `I64` and `U64` words, selected wrapping arithmetic, bitwise word operations, masked left and right shifts, discarded expression values, return latching, basic branches, and termination. A shift count is reduced to the low six bits before the host shift operation. The promoted word type selects arithmetic right shift for `I64` and logical right shift for `U64`; left shift wraps the raw 64-bit word. Both shifts remain one budgeted in-memory instruction. `IC_SHL_CONST`, `IC_SHR_CONST`, compound shift assignments, flag-bearing shifts, memory operations, calls, native code, floating point, pointers, public value producers, narrow integers, and every other opcode remain rejected. The caller must supply a positive step limit, and each executed instruction consumes one step. Preflight is bounded by the finite input graph and is not charged against that execution limit. Adding these opcodes does not change the public API, result schema, diagnostic set, or resource boundary. The interpreter cannot access the filesystem, environment, clock, processes, or network, and validation or budget failure returns no partial result.
