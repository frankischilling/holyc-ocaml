...
The supported `IC_STRLEN` internal operation retains the original numeric
binding and owned U8 pointer. Every reached byte probe checks the object's
lifetime, extent and initialization state, and consumes an execution step
before access. The terminating zero is charged; scanning stops there. Native
emission uses the same bounded probes through its checked descriptors and keeps
its result in a private frame slot. Function names grant no operation authority,
and this path exposes no host library call or raw address. See
[internal byte-string length](docs/internal-strlen.md).
