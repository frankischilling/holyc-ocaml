...
`holyc_lib` exposes the supported high-level entry points. `preprocess_detailed` returns tokens and every diagnostic so callers can keep nonfatal warnings; the older `preprocess` convenience result remains available when only fatal errors matter. The command-line program calls the same detailed path exercised by the CLI golden tests.
