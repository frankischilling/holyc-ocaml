...
- `Frontend` owns token definitions, the audited keyword table, streaming lexical state, canonical include resolution, nested file and definition frames, the current `#include` and `#define` preprocessor slice, and stable token and definition dumps. Each preprocessing stream owns its mutable cursors and generated-byte budget. The session owns source IDs and the source-ordered definition environment.
