...
- Added bounded declaration-time I64/U64 defaults to native source functions.
  ...
  under an exact source/preparation/call certificate. Reports retain preparation
  work and saved payload bytes independently of native execution, with positive
  work/storage limits and explicit rejection of unsupported expressions and
  interleaved declaration execution.
...
- Added native fixed direct I64/U64 source functions and scalar automatic locals
  ...
  image metrics. Broader storage and the complete HolyC ABI remain later gates;
  bounded scalar defaults extend this path separately.
...
- Added integer top-level execution through `holyc run --target=ir` and program graphs through `holyc dump-ir --program`. Blocks, if/else, while, do/while, for and break now use verified control flow with conditional AND/OR short-circuiting and a shared instruction budget. Runtime errors retain their stage and instruction context. General program state, indirect/external execution, output, compile-time execution and native backends remain unfinished.
...
- Extended `Ir.Integer_interpreter` with the zero-flag direct-value forms of `IC_AND_AND`, `IC_OR_OR`, and `IC_XOR_XOR`. Each instruction accepts all four internal `I64`/`U64` operand pairings, treats only an exact zero word as false, and produces a normalized internal-`I64` zero or one. The operands must already have been produced; source-level short-circuit behavior, operand side effects, and evaluation order remain outside this execution seam. The existing binary path still owns operand order, preflight diagnostics, one-step accounting, aliasing, composition, and return behavior. The public API, `HCIRVM` diagnostic set, `holyc-ir-integer-execution-v1` schema, and registered inventory of ten ordinary cases and two properties are unchanged. The existing word-operation case checks all 48 opcode, type-pairing, and truth-pairing combinations plus a ten-step composition graph, while the binary property keeps 500 iterations. Floating and mixed-floating logical operations, public and narrow integer producers, pointers, aggregates, payloads, nonzero flags, `IC_PUSH_CMP`, chained comparisons, `IC_BR_AND_AND_*` and `IC_BR_OR_OR_*`, branch rewrites, folding, source execution, optimization, and backend emission remain unsupported.
- Extended `Ir.Integer_interpreter` with zero-flag internal-word `IC_EQU_EQU`, `IC_NOT_EQU`, `IC_LESS`, `IC_GREATER_EQU`, `IC_GREATER`, and `IC_LESS_EQU`. Each instruction consumes two internal `I64` or `U64` operands and produces a normalized internal-`I64` zero or one. Equality compares the raw 64-bit payloads. Ordered comparisons use signed order only for `I64` with `I64` and unsigned order whenever either operand is `U64`. The existing binary path still owns operand order, preflight diagnostics, one-step accounting, aliasing, composition, and return behavior. The public API, `HCIRVM` diagnostic set, `holyc-ir-integer-execution-v1` schema, and registered inventory of ten ordinary cases and two properties are unchanged. Floating and mixed-floating comparisons, public and narrow integer producers, pointers, aggregates, payloads, nonzero flags, chained comparisons, comparison-consuming branch rewrites, constant folding, source execution, optimization, and backend emission remain unsupported.
- Extended `Ir.Integer_interpreter` with zero-flag internal-word `IC_SHL` and `IC_SHR`. Both operations use the low six bits of the right operand as the shift count and produce internal `U64` when either operand is `U64`, otherwise internal `I64`. Left shift wraps the raw 64-bit word; right shift is arithmetic for promoted `I64` and logical for promoted `U64`. The existing binary path still owns operand order, preflight diagnostics, one-step accounting, composition, and return behavior. The public API, `HCIRVM` diagnostic set, and `holyc-ir-integer-execution-v1` schema are unchanged. The existing word-operation case and 500-case binary property now cover both shifts without changing the registered inventory of ten ordinary cases and two properties. `IC_SHL_CONST`, `IC_SHR_CONST`, compound shift assignments, flag-bearing shifts, floating and public value producers, narrow integers, and pointers remain unsupported.
- Extended `Ir.Integer_interpreter` with zero-flag internal-word `IC_AND`, `IC_OR`, and `IC_XOR`. Each operation preserves all 64 payload bits and produces internal `U64` when either operand is `U64`, otherwise internal `I64`. The existing binary path still owns operand order, preflight diagnostics, one-step accounting, composition, and return behavior. The public API, `HCIRVM` diagnostic set, and `holyc-ir-integer-execution-v1` schema are unchanged. The existing word-operation case and 500-case binary property now cover all three operations and all four `I64`/`U64` operand pairs without changing registered test counts.
- Fixed canonical IR validation so an instruction cannot read its own result. Unary and binary self-references report `HCIR0008` before graph construction or interpreter execution, including instructions in unreachable blocks. Prior-value reuse and duplicate-ID diagnostics retain their existing behavior.
- Added `Ir.Integer_interpreter`, a deterministic library interpreter for a source-audited integer subset of checked `Ir.X87_stack.t` graphs. Preflight validates every instruction, including unreachable blocks, before execution. The subset covers internal zero-depth `I64` and `U64` immediates, exact unary rules, wrapping addition, subtraction, and multiplication, discarded expressions, return latching, direct jumps, zero and nonzero branches, `IC_RET`, and `IC_END`. A required positive `max_steps` charges every executed instruction, permits termination on the final step, and stops loops with a structured diagnostic. Empty-block fallthrough costs no step; block transfers clear local values but preserve the pending return. Results retain an optional typed return or stream termination and have a deterministic versioned renderer. The interpreter is not connected to HolyC source, the CLI, `#exe`, native code, or external resources.
- Aligned canonical checked unary-minus typing with the pinned optimizer. Integer and character target words whose signed `int64` representation is negative use internal `U64`; unary minus over that exact zero-depth type produces internal `I64` while retaining its `U64` operand. Function-body and executable top-level lowering now emit the corresponding `IC_IMM_I64`/`IC_UNARY_MINUS` pair in JIT and AOT mode, and the pair enters verified integer-unary folding, including the wrapping `Int64.min_int` case. Public `U64`, narrower integers, pointers, and existing `I64`, `F64`, logical-not, and complement rules are unchanged.
- Added verified integer-unary constant folding over `Ir.X87_stack.t`. A source-ordered block scan folds a sole-use, same-block, zero-flag internal `I64` or `U64` `IC_IMM_I64` through zero-flag `IC_COM`, `IC_NOT`, or `IC_UNARY_MINUS`, preserving the operator instruction, result, target type, and span while removing only its consumed producer. Nested eligible chains reach a fixed point, and a second run leaves the graph unchanged with an empty trace. The pass rebuilds the block graph, reruns x87 verification, and publishes a deterministic rewrite log only after both checks succeed. Shared operands, flags, other widths, public integer types, `F64`, binary folding, cross-block propagation, dead-code elimination, scheduling, interpretation, and machine emission remain separate work.
- Lowered exact checked direct-function addresses through the canonical expression planner. Resolved JIT functions emit symbolic `IC_IMM_I64`, resolved AOT functions emit symbolic `IC_ABS_ADDR`, and unresolved JIT externs emit `IC_IMM_I64` followed by an operand-linked `IC_DEREF`. Every result keeps the canonical function identity, internal zero-depth `RT_PTR` type, exact `&` operator span, and consecutive identities. Checked conversion intent and direct-call `ICF_PUSH_RES` apply only to the final producer. Ordinary object and callback addresses, bare function values, numeric address resolution, runtime slot access, relocation and fixup records, execution, and machine emission remain separate work.
- Added checked frame-relative address fragments for exact bound object identifiers. Parameters, synthetic `argc` and `argv`, and automatic scalar, pointer, array, by-value aggregate, and zero-sized object locations emit `IC_RBP`, signed `IC_IMM_I64`, and `IC_ADD` with one derived pointer type, exact spans, and consecutive identities. Static, function-pointer, module, outer, maximum-pointer-depth, and unsupported positive parameter-array paths return an explicit unsupported result. Loading, storing, index scaling, expression-planner integration, execution, and machine emission remain separate work.
- Lowered checked standalone `offset(Class.member...)` expressions through the canonical expression planner. Function-body and executable top-level forms emit one zero-operand internal-`I64` `IC_IMM_I64` from their retained cumulative path, with the exact expression span, zero flags, and consecutive identities. Grouping, integer arithmetic, mixed-`F64` conversion intent, and `IC_END_EXP` share the existing JIT and AOT path. The lowerer validates the complete semantic evidence before allocating identities and does not repeat lookup or layout arithmetic. Local and global object roots, runtime addresses, interpretation, and execution remain separate work.
- Resolved standalone `offset(Class.member...)` expressions to checked aggregate paths in function bodies and executable top-level statements. Each result retains the keyword, balanced wrappers, source root, exact visible aggregate publication, source-ordered member spellings and origins, checked current-before-base lookups, intermediate types, cumulative byte offsets, and final internal-`I64` value. Direct, inherited, nested, grouped, and numerically composed forms behave identically in JIT and AOT mode. Unknown or incomplete roots, missing members, and invalid pointer or scalar continuations report `HCSEMA0046` without a partial result. Immediate IR lowering remains separate work.
- Lowered resolved function-body breaks into checked control-flow fragments. Every loop, switch, and subswitch region receives a consecutive caller-seeded block identity, including regions with no break. Each source-ordered break emits a zero-operand `IC_JMP` to its resolved region with the exact statement span and zero flags. Block placement, complete loop and switch graphs, optimization, interpretation, and backend emission remain separate work.
- Resolved function-body `break` statements to stable function-local control-region identities. `while`, `do while`, `for` bodies, switches, and subswitches each establish a region; nested blocks, sequences, conditionals, locks, and try/catch preserve the nearest target. Direct breaks without a region, including `for` initializers and updates, are rejected. Complete loop and switch lowering and execution remain separate work.
- Lowered resolved function-local language labels and gotos into canonical fragments. Each unique language label receives a stable caller-seeded block identity in first-occurrence order; source-ordered definitions emit `IC_LABEL`, and references emit `IC_JMP` to that same block. Both instructions keep their exact occurrence span and zero flags. Assembly forms, repeated definitions, block partitioning, full graph construction, execution, and code generation remain separate work.
- Lowered checked function returns into canonical fragments. A present value is followed by typed `IC_RETURN_VAL`; every return then appends `IC_JMP` to a caller-owned leave block. Integer-to-`F64` and `F64`-to-integer root conversion intent remains on the final producer, while complete return spans and consecutive identities are preserved in JIT and AOT mode. Leave-block placement, active-try unwinding, epilogues, interpretation, and code generation remain separate work.
- Lowered checked function and executable top-level conditions into branch fragments. `if`, `while`, and `for` append `IC_BR_ZERO`; `do while` appends `IC_BR_NOT_ZERO`. Each branch consumes the condition expression's final value, keeps the caller-owned block target and retained condition or root span, and advances only the instruction identity in JIT and AOT mode. Block placement, fallthrough construction, loops, statement bodies, top-level entry records, interpretation, and code generation remain separate work.
- Retained fixed-parameter conversion intent on provided function-body and executable top-level argument roots, then lowered it onto each root's final producer. Integer roots carry `ICF_RES_TO_F64`; `F64` roots carry `ICF_RES_TO_INT`. Grouping and unary plus add no instruction, and literals and composite trees keep their checked payloads, types, spans, operands, and identities in JIT and AOT mode. Numeric conversion, call lowering, optimization, and execution remain separate work.
- Lowered pure and mixed `F64` shift expressions through `IC_SHL` and `IC_SHR`. Each operation keeps an internal-`F64` result and zero operation flags; a retained integer operand or integer shift subtree result carries `ICF_RES_TO_F64` without changing its checked type or payload. Function bodies and executable top-level roots preserve source postorder, exact links, spans, and identities in JIT and AOT mode. Shift evaluation, constant-form rewrites, optimization, interpretation, and backend emission remain separate work.
- Lowered pure and mixed `F64` bitwise expressions through `IC_AND`, `IC_OR`, and `IC_XOR`. Each operation keeps an internal-`F64` result and zero operation flags; a retained integer operand or integer subtree result carries `ICF_RES_TO_F64` without changing its checked type or payload. Function bodies and executable top-level roots preserve source postorder, exact links, spans, and identities in JIT and AOT mode. Raw-bit evaluation, immediate conversion, optimization, interpretation, and backend emission remain separate work.
- Lowered pure and mixed `F64` logical binary expressions through `IC_AND_AND`, `IC_OR_OR`, and `IC_XOR_XOR`. Each operation produces internal `I64`; an integer operand or nested logical result consumed in the floating domain carries `ICF_RES_TO_F64` on its retained producer. Function bodies and executable top-level roots share source postorder, exact links, spans, and identities in JIT and AOT mode. Constant truth evaluation, branch rewrites, short-circuit control flow, interpretation, and backend emission remain separate work.
- Lowered pure and mixed `F64` modulo expressions to `IC_MOD`. Integer operands retain their checked type and payload while their producer receives `ICF_RES_TO_F64`; floating operands and the modulo instruction remain unflagged. Nested and grouped trees preserve source postorder, exact operand links, spans, and module-owned identities in function bodies and executable top-level code under JIT and AOT. Folding, numerical remainder evaluation, floating backend selection, interpretation, and execution remain separate work.
- Lowered checked function-body aggregate offset paths through the canonical expression planner. Each completed path emits one zero-operand internal-`I64` `IC_IMM_I64` with its final cumulative byte offset and exact source span. Direct, inherited, nested, grouped, numeric, mixed-`F64`, and expression-statement uses now share the same JIT and AOT path as executable top-level offsets. Ordinary object-member access and incomplete or inconsistent evidence expose no partial IR.
- Typed source-visible aggregate offset paths in function-body expressions. Direct, inherited, nested, and grouped paths now retain the exact aggregate publication, current-or-base member lookups, intermediate types, and cumulative byte offsets while producing the internal-`I64` value selected by `PrsOffsetOf`. Numeric composition keeps that evidence in JIT and AOT mode. Invalid paths report `HCSEMA0046` without a partial checked result; runtime addresses and execution remain separate work.
- Lowered checked executable top-level aggregate offset paths through the canonical expression planner. Each completed path emits one zero-operand internal-`I64` `IC_IMM_I64` with its final cumulative byte offset and exact source span. Direct, inherited, nested, grouped, integer, mixed-`F64`, and expression-statement uses share the same JIT and AOT path. Ordinary object-member access and incomplete or inconsistent offset evidence expose no partial IR.
