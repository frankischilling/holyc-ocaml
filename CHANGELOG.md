...
- Added primitive automatic and static local declarations in function bodies, including pointer layers, arrays, scalar initializers, per-declarator register requests, function-wide parser visibility, deterministic dumps, and explicit diagnostics for unsupported local forms.
