...
- Added checked `StreamExePrint` for compiled AOT `#exe` execution. Formatting
  ...
  JIT calls format before reporting the compiled-mode diagnostic. Source bytes,
  steps, frames and call depth retain exact limits without publishing the
  executed text as StreamPrint-generated bytes.
...
- Aggregate `$$` expressions now use original local positions after runtime-sized
  ...
  The runtime-frame fixture returns 42 in both outer modes at 74 runtime steps
...
- Aggregate `$$` expressions now capture named JIT local declaration positions
  ...
  42 in both modes at 28 runtime steps and three preparation units. Aggregate-valued
...
- Aggregate `$$` expressions now capture named JIT function sizes at original
  ...
  The new CLI fixture returns 42 in both outer modes at 47 runtime steps and
...
- Retained aggregate offsets now observe shared compiler-position writes from
  ...
  shared-position CLI fixture returns 42 in both modes at 35 runtime steps and
...
- Added original aggregate-position evidence to mixed integer runtime `$$`
  ...
  outer position. The new CLI fixture returns 42 in both outer modes at 47
...
- Added typed integer runtime offsets for retained task aggregates. Original
  ...
  fixture returns 42 in both outer modes at 50 runtime steps and three
...
- Prepared closed aggregate `$$` offsets at their original parser phases.
  ...
  both modes at 28 runtime steps and six preparation units, with one-below checks.
