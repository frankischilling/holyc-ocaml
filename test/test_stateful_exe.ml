...
let stream_exe_print_is_not_bridged () =
  List.iter
    (fun mode ->
      let report = Output.run ~mode {|#exe {StreamExePrint("42;");}|} in
      ignore (Output.fault "HCPARSE0001" report);
      Alcotest.(check bool)
        "unavailable StreamExePrint has no successful program result" true
        (Result.is_error (integer_program_report_outcome report));
      let generated_bytes =
        integer_program_report_progress report
        |> Option.fold ~none:0 ~some:(fun (progress : Integer_task.progress) ->
            progress.runtime.generated_bytes)
      in
      Alcotest.(check int)
        "unavailable StreamExePrint commits no generated bytes" 0
        generated_bytes)
    G.modes
...
let tests =
  List.map
    ...
    gates
  @ [
      Alcotest.test_case "StreamExePrint stays outside hosted #exe bridge"
        `Quick stream_exe_print_is_not_bridged;
      Alcotest.test_case
