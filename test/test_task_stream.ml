...
let expanded_formatting () =
  List.iter
    (fun (case : Integer_format_fixture.t) ->
      ...
      Alcotest.(check int)
        (case.label ^ " generated capacity")
        (String.length case.bytes)
        (Task.generated_bytes task);
      Alcotest.(check int)
        (case.label ^ " shared formatting work")
...
let expanded_format_limits () =
  List.iter
    (fun work_limit ->
      let session, task =
        create ~max_generated_bytes:8 ~max_output_bytes:5
          ~max_output_work:work_limit ()
      in
      let stream = begin_ task in
      run session task {|StreamPrint("%0*tX",8,0x10000002A);|};
      if work_limit = 28 then run session task {|Print("%5s","AB");|}
      else T.fault "HCIRVM0023" (T.run session task {|Print("%5s","AB");|});
      Alcotest.(check string)
        "formatted generation survives later ordinary work" "0000002A"
        (finish task stream);
      Alcotest.(check int)
        "generated bytes stay independent" 8
        (Task.generated_bytes task);
      Alcotest.(check int)
        "expanded formats share exact work" work_limit (Task.output_work task);
      Alcotest.(check string)
        "ordinary formatted draft is atomic"
        (if work_limit = 28 then "   AB" else "")
        (Task.output_bytes task))
    [ 28; 27 ];
  let session, task = create ~max_generated_bytes:1 () in
  let stream = begin_ task in
  T.fault "HCIRVM0028"
    (T.run session task {|StreamPrint("%*d",9223372036854775807,1);|});
  Alcotest.(check int)
    "huge generated width has bounded append work" 5 (Task.output_work task);
  Alcotest.(check string)
    "huge generated draft commits no prefix" "" (finish task stream);
  let session, task = create ~max_generated_bytes:0 ~max_output_work:8 () in
  let stream = begin_ task in
  run session task {|StreamPrint("%0ts","AB");|};
  Alcotest.(check string)
    "truncation permits zero generation capacity" "" (finish task stream);
  Alcotest.(check int)
    "zero generation still performs full scan" 8 (Task.output_work task)

let interleaved_format_faults () =
  List.iter
    (fun (label, format, arguments, work) ->
      List.iter
        (fun (work_limit, code, expected_work) ->
          let session, task =
            create ~max_generated_bytes:1 ~max_output_work:work_limit ()
          in
          let stream = begin_ task in
          let case =
          ...
          T.fault code
            (T.run session task
               (Integer_format_fixture.call "StreamPrint" case));
          Alcotest.(check string)
            (label ^ " atomic generation")
            "" (finish task stream);
...
let tests =
  [
    Alcotest.test_case "shared integer formats and byte padding" `Quick
      expanded_formatting;
    Alcotest.test_case "expanded formats keep generation and work bounds" `Quick
