...
The corresponding `Function_prototype` AST node requires one of the five supported bindings and a semicolon. It keeps the ordered prefix nodes, return type and pointer layers separate from the function name, then records both parentheses, every supported parameter type, its register qualifiers and pointer layers, optional parameter names and defaults, commas, a distinct terminal variadic marker, and the semicolon. `extern U8 *CmdLinePmt();`, `public _extern _SYS_HLT U0 SysHlt();`, and `_intern Bsf I64 Bsf(I64 bit_field_val);` are pinned examples. `Compiler/CExts.HC` supplies named class returns and parameters such as `extern CDoc *DocNew(...)` and `extern CHashClass *PrsClass(CCmpCtrl *cc,I64 keyword)`. Accepted prototypes enter the streaming symbol environment as functions only after the complete semicolon is present.
...
The dot and `TK_DEREFERENCE` branches of `PrsUnaryModifier` share a member-name path and return to the same modifier state. The AST distinguishes `object.field` from `pointer->field` and supports chains such as `Factory().nodes[0]->callback(1).result`. The semantic source view keeps the complete base expression, access kind, operator origin, member spelling, and member origin. Direct access requires a known aggregate object; pointer access removes one checked pointer layer before requiring the aggregate. `MemberFind` searches the current class before its one base chain. The immutable lookup result retains the queried aggregate, declaring aggregate, inheritance depth, member symbol, resolved type, completed byte offset, array dimensions, and callback flag. Array members remain array values until indexing consumes their dimensions, callback members remain callback values, and ordinary fields become lvalues only when an address or update parent requests one. Aggregate field types follow backing headers visible before the caller. `HCSEMA0046` reports a proven invalid base at the operator, a missing or premature member at its name, and a foreign member index before producing a partial result. An unavailable outer base remains unresolved. Offset addition, storage reads, indirect callback calls, and IC lowering are not implemented. [Issue #270](https://github.com/frankischilling/holyc-ocaml/issues/270) records this boundary.
...
This slice parses but does not execute default, dimension, `_intern`, call, index, member, postfix update, cast, `sizeof`, `offset`, `defined`, or global-initializer expressions. It does not apply type or cast conversions, copy string defaults, evaluate selected direct-call defaults, bind or check indirect calls, substitute the previous explicit argument's class name for `lastclass`, evaluate array sizes or initial values, validate an index or member base, scale an index, calculate member layout, dereference an element or pointer member, apply an increment or decrement, resolve a `sizeof` or `offset` target, calculate a size or member offset, resolve a `defined` name or choose its Boolean result, assign storage or executable addresses, or set internal-function state. The semantic call pass checks direct fixed and variadic slot shape, but not expression types or values. Parenthesis-free syntax requires a visible direct function entry with retained call shape; function-pointer globals and locals remain variable entries. Unrecognized cast targets remain outside this expression slice and receive `HCPARSE0020` with wording for the active context. `HCPARSE0024` reports a missing comma between parenthesized call arguments, `HCPARSE0025` reports an unclosed call, `HCPARSE0026` reports an unclosed index, `HCPARSE0027` reports a member operator without a following identifier, `HCPARSE0028` reports a suffix after a terminal postfix update, `HCPARSE0029` rejects C-style prefix casts, `HCPARSE0030` reports a cast without its closing parenthesis, `HCPARSE0031` through `HCPARSE0034` cover malformed `sizeof` forms, `HCPARSE0035` through `HCPARSE0039` cover malformed `offset` forms, `HCPARSE0040` through `HCPARSE0042` cover malformed `defined` forms, and `HCPARSE0105` and `HCPARSE0106` cover incomplete direct-call shape. Array parameters remain unavailable. Function definitions establish a function-wide parser lookup context for parameters, variadic `argc` and `argv`, and accepted local names, but that context is not a semantic scope and carries no storage or type-checking state. Global and static-local scalar or recursive braced initializers are retained as syntax, but are not type-checked, evaluated, or assigned storage. Direct callback-local initializers, automatic local braced initializers, and other unsupported declarator forms are not stored as raw tokens. General unsupported global syntax uses `HCPARSE0001` through `HCPARSE0003`; a fifth star uses `HCPARSE0004`; malformed function-pointer parameter punctuation uses `HCPARSE0014`; global callback failures use `HCPARSE0131` and `HCPARSE0132`; member callback failures use `HCPARSE0133` and `HCPARSE0134`; local callback failures use `HCPARSE0135` through `HCPARSE0137`; static-local initializer failures use `HCPARSE0138` through `HCPARSE0141`; and a missing alternate target uses `HCPARSE0007`. Recovery advances to a semicolon or EOF, and the public result contains no AST after any error.
...
Immediate syntax uses the ordinary HolyC expression parser without parenthesis-free function calls. Direct bracketed addresses stay lossless rather than being forced into the language-expression AST. Visible local or newer compiler symbols can shadow opcode and register spellings. Exact excerpts from `Adam/AMem.HC`, `Kernel/Job.HC`, `Demo/Graphics/Balloon.HC`, `Demo/Asm/AsmAndC3.HC`, `Demo/Lectures/Ring3.HC`, and `Kernel/KInts.HC` parse in JIT and AOT modes.
...
The compiler still does not resolve types in casts outside the documented direct-call source view, or resolve `sizeof` or `offset`; bind or evaluate defaults nested in callback members; create semantic block scopes; compare function headers; allocate runtime storage; evaluate `_intern` addresses, defaults, initializers, symbol-dependent aggregate offsets, or metadata; resolve alternate binding addresses; create fixups or loader records; complete task-level linkage; honor register requests; fill default call arguments; resolve or lower implicit output calls; validate break and return targets; bind AST member expressions to the available layout index; resolve assembly operand references or repeated assembly definitions by output address; validate assembly operands or directives; evaluate or apply assembler state and output effects; evaluate switch cases or build jump tables; construct executable control flow; implement exception transfer; execute top-level code; or support whole-value backing semantics, subinteger access, metadata lookup, direct local callback initialization, automatic local braced initializers, remaining control flow, promotions, value conversions, floating execution, compile-time compiler-option changes, or structured semantic diagnostics. Those claims remain absent from the compatibility report until their own source-grounded tests pass.
