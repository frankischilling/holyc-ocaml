...
At top level, a known global or function name can begin an expression statement. An unresolved identifier still takes the declaration or label path used by `PrsStmt`; label and declaration syntax after a sequence comma is not implemented yet. The same sequence representation is used recursively inside compound blocks. Control flow, local declarations, labels, and function-body statements remain separate parser work. `HCPARSE0047` reports a missing expression-statement terminator. `HCPARSE0048` reports a statement form at a comma boundary that this slice cannot represent.
