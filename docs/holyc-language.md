...
global-declaration := declaration-modifier* primitive declarator ("," declarator)* ";"
...
Empty and comment-only inputs are valid modules. Declarations retain source order, the exact modifier, type, and identifier spellings, and ordered source segments. A comma-separated group has one shared modifier list and primitive type, plus a source-linked declarator for every name. Each `public` or `static` prefix is a separate ordered AST node. Each declarator records its terminating comma or semicolon explicitly. Each pointer star is a separate layer numbered from one through four, and numbering starts again for every declarator. A modifier, layer, or delimiter keeps its raw spelling, generated frame, invocation span, and definition span. Includes and definitions run through the integrated stream before parsing. A parser diagnostic produced inside an include retains its include stack, while a diagnostic on definition-generated text records both the expansion and declaration sites.

`Compiler/PrsStmt.HC:PrsStmt` handles `KW_PUBLIC` and `KW_STATIC` before it recognizes a type and calls `PrsGlblVarLst`. The staged flag assignments are order-sensitive: `public` retains calling, public, and assembly state, while `static` keeps assembly state and clears the other staged bits. `PrsGlblVarLst` later consults the public bit when it registers a global name. The current syntax tree preserves the ordered prefix tokens but does not yet perform that linkage or storage work. The routine calls `PrsType` again after a comma while passing the saved base class and mode. `Compiler/PrsVar.HC:PrsType` counts stars and rejects a depth greater than `PTR_STARS_NUM`; the pinned `Kernel/KernelA.HH` sets that value to four. `Compiler/PrsLib.HC:PrsClassNew` creates the matching depth-zero through depth-four class records. The syntax tree records the pointer layers and list boundaries, but semantic pointer types, layout, dereference behavior, and conversions are not implemented yet.

The current list grammar accepts forms found in the pinned corpus such as `I64 prime_range,my_mp_cnt,pending;`, `I32 *arg1,*arg2;`, and `U8 *ptr,**idx;`. It also accepts ordered and repeated `public` or `static` prefixes. Each completed name is published before the parser asks the integrated stream for the next token, so a following conditional can observe it even when the directive appears after a comma. Initializers, parenthesized function pointers, array suffixes, `extern`, `import`, register modifiers, and other declarator forms are not silently stored as raw tokens. General unsupported syntax uses `HCPARSE0001` through `HCPARSE0003`; a fifth star uses `HCPARSE0004`, and a parenthesized function pointer uses `HCPARSE0005`. A modifier without a supported primitive type receives `HCPARSE0001` at the following token. Recovery advances to a semicolon or EOF, and the public result contains no AST after any error.
