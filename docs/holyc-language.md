...
aggregate-definition := declaration-modifier* aggregate-backing? aggregate-keyword identifier
                        aggregate-base? "{" aggregate-member* "}" ";"
...
global-declaration := declaration-modifier* declaration-binding? type-specifier declarator ("," declarator)* ";"
...
The definition AST retains ordered modifiers, an optional backing type with zero through four pointer layers, the distinct aggregate kind, keyword, name, an optional base, both braces, every member, and the final semicolon. The base record keeps the colon and identifier spellings and locations separately. An ordinary member group keeps one primitive, intrinsic, or visible named type plus independent pointer layers, array dimensions, and comma or semicolon delimiters for each name. An anonymous union owns its recursive member list and records whether a semicolon followed its closing brace. Empty member semicolons remain explicit nodes. The aggregate name enters the parser environment before the base token or members are read, matching the source publication point. Both compile modes share this syntax, and generated definitions retain their invocation and definition locations. `HCPARSE0121` rejects a missing or nonclass base, while `HCPARSE0126` reports a second base. Other `HCPARSE0109` through `HCPARSE0125` diagnostics cover malformed bodies, deferred forms, unknown backings, and a binding before a backed definition. A hosted limit of 256 nested anonymous unions prevents parser exhaustion.

The six public integer unions at `Kernel/KernelA.HH:67-113` parse with their exact intrinsic backings. Tests also cover `CDate`, `CICType`, `CAbsCntsI64`, the backed color wrappers, all 17 sprite definitions at `Adam/Gr/Gr.HH:18-115`, and representative kernel inheritance chains. A byte-preserving text scan finds 37 inherited class headers and no inherited union header in the pinned Compiler, Kernel, Adam, and Demo `.HC` and `.HH` set. Four of those headers occur in files that ordinary text-mode searches treat as binary, so the scan forces byte input. The shared source path still permits a union base, which has a focused syntax test but no corpus claim. These nodes do not assign semantic type identity, validate a backing, add inherited size, resolve inherited members, calculate offsets or storage, overlap union members, apply alignment, or implement metadata and subinteger access. Explicit `$$` offsets, nested named aggregate definitions, a trailing global object declarator, and function-pointer members remain separate parser gates.
