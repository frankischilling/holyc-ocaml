...
`PrsDotDotDot` adds `argc` followed by `argv` after the fixed members. Both use internal `I64`; `argc` is scalar and `argv` is array-shaped. The source writes no `argv` extent, while the compiler stores 127 as a placeholder dimension. The semantic model keeps both facts instead of reporting 127 as a source bound or treating `argv` as a C `char **`. Only a top-level function ellipsis receives those synthetic symbol bindings; an ellipsis nested inside a callback retains its signature marker. Prototype/definition reconciliation, call checking, default evaluation, function flags, register requests, storage, ABI behavior, and linkage remain separate work under [issue #181](https://github.com/frankischilling/holyc-ocaml/issues/181).
...
The semantic library assigns stable module, aggregate, and function scopes and symbols; reconciles source-ordered aggregate identities; resolves definition headers and direct member heads; resolves top-level function returns and recursive fixed-parameter signatures; and binds function-local language gotos to language or structural assembly-block labels. These APIs validate their complete AST and semantic batches before returning and keep the symbol table unchanged during each resolution pass. They are library boundaries rather than a claim that the whole parsed program has been checked.

The compiler still does not resolve types in globals, locals, member callbacks, casts, `sizeof`, or `offset`; create semantic block scopes; reconcile function or global declarations; allocate storage; evaluate `_intern` addresses, defaults, initializers, aggregate-offset directives, or metadata; apply linkage or storage effects; calculate array or aggregate layout; honor register requests; fill default call arguments; resolve or lower implicit output calls; validate break and return targets; resolve assembly operand references or repeated assembly definitions by output address; validate assembly operands or directives; evaluate or apply assembler state and output effects; evaluate switch cases or build jump tables; construct executable control flow; implement exception transfer; execute top-level code; or support whole-value backing semantics, subinteger access, inherited layout or member lookup, metadata lookup, direct local callback initialization, automatic local braced initializers, remaining control flow, promotions, conversions, floating execution, compiler-option effects, or structured semantic diagnostics. Those claims remain absent from the compatibility report until their own source-grounded tests pass.
