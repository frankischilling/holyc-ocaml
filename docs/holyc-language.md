...
The AST keeps the storage class, shared primitive or named type specifier, and every declarator in source order. Each declarator records its own register request, optional explicit register, pointer layers, array dimensions, initializer, and delimiter. `Adam/Snd/SndMath.HC` supplies a comma group with repeated `reg`; `Demo/Asm/AsmAndC1.HC` and `Demo/Asm/AsmAndC2.HC` show `noreg` and explicit `R15`; and `Kernel/KMisc.HC` supplies a grouped static initializer. Named parameters enter the same parser context before the body. Variadic functions also expose `argc` and `argv`, matching the synthetic members created by `PrsDotDotDot`.

The lookup context spans the whole parsed function, including nested blocks. The parser uses it to route later identifiers and symbol conditionals; it does not assign offsets, allocate storage, enforce block lifetime, perform semantic shadow checks, honor a register request, evaluate an initializer, or calculate array sizes. A local entry shadows a class symbol for subsequent syntax dispatch, matching the existing environment lookup. Local function-pointer declarators and local braced initializers remain unsupported. `HCPARSE0098` through `HCPARSE0104` report malformed or unsupported local forms without returning a successful AST. Static locals reject the automatic after-type `reg` and `noreg` grammar with `HCPARSE0099`.
