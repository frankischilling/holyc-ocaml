...
The AST represents this syntax as a dedicated aggregate forward item. It distinguishes class from union, retains the optional ordered modifiers, ordinary `extern` binding, aggregate keyword, name, semicolon, and composite source provenance, and publishes the accepted name as a class symbol before reading later input. JIT and AOT parser modes accept the same shape. `HCPARSE0107` reports a missing name, while `HCPARSE0108` reports a missing semicolon, a comma list, or a body. Rejected names are not published. Parsing a body, using a forwarded name as a declaration type, inheritance, members, layout, metadata, and semantic duplicate rules remain separate work.
