...
`Sema.Function_call_expression_result` now performs the source-result half before target conversion. It assigns deterministic IDs while walking provided fixed arguments and keeps the source origin, known semantic type, value category, remaining array rank, intrinsic conversion requirement, and forwarded result class in one immutable record. A nested identifier used by address-of or an update remains an lvalue; the completed parent is an address or object value. Prefix dereference follows the pointer-class adjustment at `PrsExp.HC:145-154`: it removes one semantic pointer layer without changing the public, intrinsic, or aggregate base. When the source class has no pointer stars, the class remains unchanged; `OptPass012.HC:275-289` handles the separate `U0` warning later. An array already supplies an address in the pinned parser, so dereferencing it yields its declared element type without subtracting a stored pointer layer. `PrsPopDeref` removes the pending data dereference for a callback, so that result is a function value. The dereference node remains an lvalue beneath address-of or an update and becomes an object value in an ordinary call argument. An outer value with no checked type remains unavailable instead of becoming a guessed pointer. `Sema.Function_call_conversion_decision` consumes the exact root result and records target-specific conversion intent separately. Results are owned by the semantic session, aggregate member index, and policy traversal that produced them. Assignment, indirect-call, default, and variadic typing remain outside this boundary.
...
Binary source views retain the generated `IC_*` identity and both recursive operands. `OptPass012` gives power an `F64` result and gives logical or comparison operators an integer result. Its shared binary fixup chooses the higher raw class for arithmetic, shift, and bitwise operators, so an audited `F64` operand selects the floating path, two audited integer-side operands select the integer path, and an unavailable operand keeps the result unresolved unless the other operand already forces `F64`. Assignment families remain unresolved because their result and conversion depend on the typed lvalue. This is result-class analysis only; it does not fold constants, validate operands, apply side effects, or emit IC.
