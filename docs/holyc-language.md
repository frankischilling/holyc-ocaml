...
aggregate-definition := declaration-modifier* aggregate-keyword identifier
                        "{" aggregate-member* "}" ";"
...
The definition AST retains ordered modifiers, the distinct aggregate kind, keyword, name, both braces, every member, and the final semicolon. An ordinary member group keeps one primitive or visible named type plus independent pointer layers, array dimensions, and comma or semicolon delimiters for each name. An anonymous union owns its recursive member list and records whether a semicolon followed its closing brace. Empty member semicolons remain explicit nodes. The aggregate name enters the parser environment before its members are read, which permits self-referential pointers and makes a completed forward usable without a second symbol category. Both compile modes share this syntax, and generated definitions retain their invocation and definition locations. `HCPARSE0109` through `HCPARSE0123` cover malformed bodies and deferred source forms. A hosted limit of 256 nested anonymous unions prevents parser exhaustion.

This slice does not accept a backing type before `class` or `union`, inheritance, member metadata, explicit `$$` offsets, nested named aggregate definitions, a trailing global object declarator, or function-pointer members. It also does not assign semantic type identity, clear or reconcile forward flags, compare duplicates, calculate member offsets or storage, overlap union members, apply alignment, resolve metadata, or perform linkage. These remain separate semantic and parser gates.
