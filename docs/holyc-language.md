...
A controlled native run confirms `Zero;`, an all-default call, one required argument between defaults, two adjacent required arguments, an empty variadic tail, a binary continuation, and address-of suppression. The values are recorded in `test/oracle/parenthesis-free-calls.json`. The AST uses `Parenthesis_free_call` instead of fabricating punctuation. Omitted positions have zero-width call-site locations. `HCPARSE0105` reports a missing required expression, and `HCPARSE0106` rejects a parenthesis-free call whose function entry lacks fixed-parameter shape. Explicit parenthesized calls do not need that shape. This is still syntax handling. Default evaluation and substitution, conversions, argument type checking, semantic call resolution, lowering, and execution are not implemented.
