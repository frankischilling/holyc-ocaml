...
The semantic library assigns stable module, aggregate, and function scopes and symbols; reconciles source-ordered aggregate identities; resolves definition headers, direct member heads, recursive member callbacks, global types, and top-level function signatures; retains the checked `MLF_FUN` fact on callback members; calculates closed class and union byte layouts; validates direct and inherited member names; reconciles function identities and global records; classifies their source-backed flags and consumer intent; and binds function-local language gotos to language or structural assembly-block labels. `holyc dump-layout` exposes the completed aggregate facts as deterministic human or JSON data. These APIs validate their complete AST and semantic batches before returning and keep the symbol table unchanged during each resolution or classification pass. They are library boundaries rather than a claim that the whole parsed program has been checked.
