...
For a definition, `Compiler/PrsStmt.HC:PrsClass` creates or reuses the named class entry and adds a newly created entry before its first `Lex` call. It then delegates both class and union bodies to `Compiler/PrsVar.HC:PrsVarLst`. That routine skips empty semicolons, restarts a member declaration after a comma, parses pointer and array suffixes through the shared type path, and recursively handles an unnamed `union` body. `Kernel/KernelA.HH:116-179` supplies 12 consecutive ordinary class definitions with primitive fields, named self references, grouped members, pointers, and arrays. `Doc/HolyC.DD` confirms that a named union is subsequently referenced like a class rather than with a repeated `union` label.

The definition AST retains ordered modifiers, an optional backing type with zero through four pointer layers, the distinct aggregate kind, keyword, name, both braces, every member, and the final semicolon. An ordinary member group keeps one primitive, intrinsic, or visible named type plus independent pointer layers, array dimensions, and comma or semicolon delimiters for each name. An anonymous union owns its recursive member list and records whether a semicolon followed its closing brace. Empty member semicolons remain explicit nodes. The aggregate name enters the parser environment before its members are read, which permits self-referential pointers and makes a completed forward usable without a second symbol category. Both compile modes share this syntax, and generated definitions retain their invocation and definition locations. `HCPARSE0109` through `HCPARSE0125` cover malformed bodies, deferred forms, unknown backings, and a binding before a backed definition. A hosted limit of 256 nested anonymous unions prevents parser exhaustion.

The six public integer unions at `Kernel/KernelA.HH:67-113` now parse with their exact intrinsic backings. Tests also cover `CDate`, `CICType`, `CAbsCntsI64`, and the backed color wrappers. This syntax does not assign semantic type identity, validate a backing against its body, provide whole-value conversion, calculate member offsets or storage, overlap union members, apply alignment, or implement subinteger access. Inheritance, member metadata, explicit `$$` offsets, nested named aggregate definitions, a trailing global object declarator, and function-pointer members remain separate parser gates.
