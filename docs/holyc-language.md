...
Inside a function, `Compiler/PrsStmt.HC:PrsStmt` routes a known `HTT_CLASS` or `HTT_INTERNAL_TYPE` to `PrsVarLst` instead of the global declaration path. A leading `static` selects the static-local mode. Automatic locals may place `reg` or `noreg` after the shared type on each declarator, and `reg` may consume one canonical U64 register name. `PrsType` may return ordinary pointer layers or a recursive callback signature. `PrsVarLst` then reads the local name or callback name, any array dimensions, an optional scalar initializer for an ordinary local, and either a comma or the final semicolon. It calls `MemberAdd` before parsing an ordinary initializer or requesting the following token, so a new name is visible in its own initializer and in later declarators.
