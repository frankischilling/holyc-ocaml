...
declaration-binding := "extern" | "import"
...
Empty and comment-only inputs are valid modules. Declarations retain source order, the exact modifier, binding, type, and identifier spellings, and ordered source segments. A comma-separated group has one shared modifier list, optional binding, and primitive type, plus a source-linked declarator for every name. Each `public` or `static` prefix is a separate ordered AST node. An `extern` or `import` token becomes a separate binding node because the source parser treats binding mode independently from staged modifiers. Each declarator records its terminating comma or semicolon explicitly. Each pointer star is a separate layer numbered from one through four, and numbering starts again for every declarator. A modifier, binding, layer, or delimiter keeps its raw spelling, generated frame, invocation span, and definition span. Includes and definitions run through the integrated stream before parsing. A parser diagnostic produced inside an include retains its include stack, while a diagnostic on definition-generated text records both the expansion and declaration sites.

`Compiler/PrsStmt.HC:PrsStmt` handles `KW_PUBLIC` and `KW_STATIC` before it recognizes an ordinary `KW_EXTERN` or `KW_IMPORT` binding. The staged flag assignments are order-sensitive: `public` retains calling, public, and assembly state, while `static` keeps assembly state and clears the other staged bits. The binding then selects `PRS0_EXTERN` or `PRS0_IMPORT` for `PrsGlblVarLst`. That routine rejects import mode unless the compile controller is AOT and later consults the public bit when it registers a global name. The current syntax tree preserves the ordered prefix tokens and binding mode but does not yet resolve linkage, allocate storage, or emit an import. The routine calls `PrsType` again after a comma while passing the saved base class and mode. `Compiler/PrsVar.HC:PrsType` counts stars and rejects a depth greater than `PTR_STARS_NUM`; the pinned `Kernel/KernelA.HH` sets that value to four. `Compiler/PrsLib.HC:PrsClassNew` creates the matching depth-zero through depth-four class records. The syntax tree records the pointer layers and list boundaries, but semantic pointer types, layout, dereference behavior, and conversions are not implemented yet.

The current list grammar accepts forms found in the pinned corpus such as `I64 prime_range,my_mp_cnt,pending;`, `I32 *arg1,*arg2;`, `U8 *ptr,**idx;`, and `public extern U8 *rev_bits_table,*set_bits_table;`. It also accepts ordered and repeated `public` or `static` prefixes, ordinary `extern` in every compile mode, and ordinary `import` in AOT mode. Each completed name is published before the parser asks the integrated stream for the next token, so a following conditional can observe it even when the directive appears after a comma. A JIT `import` receives `HCPARSE0006` before its declarator is parsed, so the rejected name cannot become visible. The underscored `_extern`, `_import`, and `_intern` forms, initializers, parenthesized function pointers, array suffixes, register modifiers, and other declarator forms are not silently stored as raw tokens. General unsupported syntax uses `HCPARSE0001` through `HCPARSE0003`; a fifth star uses `HCPARSE0004`, and a parenthesized function pointer uses `HCPARSE0005`. A modifier or binding without a supported primitive type receives `HCPARSE0001` at the following token. Recovery advances to a semicolon or EOF, and the public result contains no AST after any error.
...
The implemented language specification supplies immutable primitive type facts, operator tables, compiler-option facts, and syntax for primitive globals with ordered `public` or `static` prefixes, one ordinary `extern` or AOT `import` binding, and zero through four pointer stars. It enforces the source parser's AOT gate for `import`. It does not resolve bindings, apply modifier linkage or storage effects, build semantic pointer types, or support array or function declarators, initializers, expressions, statements, functions, classes, unions, promotions, conversions, aggregate layout, floating execution, compiler-option effects, or semantic diagnostics. Those claims remain absent from the compatibility report until their own source-grounded tests pass.
