...
The AST implements zero-, one-, and two-operand opcode forms. One `inline_assembly_statement` retains an adjacent opcode run. Each operation keeps its source spelling, canonical alias identity, operand list, separating comma, optional semicolon, source span, and generated-source provenance. Operands remain classified as register, immediate, or memory forms. Size prefixes such as `U64`, segment prefixes such as `FS:`, and every token inside an address remain available to later assembler passes.
...
`HCPARSE0147` rejects direct assembly at top level and points to `asm { ... }`. `HCPARSE0149` through `HCPARSE0152` cover a missing operand, missing comma, unclosed address bracket, and extra operand. Direct assembler keywords and label definitions, operand validation, instruction-form selection, lowering, encoding, and execution remain pending. A parsed operand is not evidence that the instruction would assemble.
