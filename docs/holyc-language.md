...
The semantic library assigns stable module, aggregate, and function scopes and symbols; reconciles source-ordered aggregate identities; resolves definition headers, direct member heads, global types, and top-level function signatures; reconciles function identities and global records; classifies their source-backed flags and consumer intent; and binds function-local language gotos to language or structural assembly-block labels. These APIs validate their complete AST and semantic batches before returning and keep the symbol table unchanged during each resolution or classification pass. They are library boundaries rather than a claim that the whole parsed program has been checked.

The compiler still does not resolve types in locals, member callbacks, casts, `sizeof`, or `offset`; create semantic block scopes; compare function headers; allocate storage; evaluate `_intern` addresses, defaults, initializers, aggregate-offset directives, or metadata; resolve alternate binding addresses; create fixups or loader records; complete task-level linkage; calculate array or aggregate layout; honor register requests; fill default call arguments; resolve or lower implicit output calls; validate break and return targets; resolve assembly operand references or repeated assembly definitions by output address; validate assembly operands or directives; evaluate or apply assembler state and output effects; evaluate switch cases or build jump tables; construct executable control flow; implement exception transfer; execute top-level code; or support whole-value backing semantics, subinteger access, inherited layout or member lookup, metadata lookup, direct local callback initialization, automatic local braced initializers, remaining control flow, promotions, conversions, floating execution, compile-time compiler-option changes, or structured semantic diagnostics. Those claims remain absent from the compatibility report until their own source-grounded tests pass.
