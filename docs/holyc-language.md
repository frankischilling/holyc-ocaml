...
`Sema.Function_call_resolution` gives parenthesized and parenthesis-free calls the same fixed-slot treatment. It keeps a provided expression distinct from an explicit hole and from a trailing slot absent in source. Either absent form selects the declared default when one exists; a required parameter instead produces `HCSEMA0040`. Extra fixed arguments produce `HCSEMA0041`, and holes after the fixed parameter block produce `HCSEMA0042` because the variadic loop accepts only expressions. Each direct result retains the declaration visible at that source point and the canonical function identity selected by the JIT or AOT join rule. A local or global callback variable, an aggregate spelling, or an unresolved outer name stays deferred. This stage does not evaluate the expression or default, apply the source integer or floating conversion, check an indirect call, lower the call, or execute it.

After a provided fixed expression is optimized, `PrsFunCall` forwards its class and the parameter member class through `OptClassFwd`. A scalar target whose forwarded raw type is `RT_F64` can receive `ICF_RES_TO_F64`; every other fixed target can receive `ICF_RES_TO_INT` when the actual expression is `F64`. `Sema.Function_call_conversion_policy` retains that target split. It follows by-value aggregate backing chains, while a pointer or callback stays on the integer path even when its pointee or return type is `F64`. Declared defaults and provided variadic expressions keep their separate source paths. The actual expression class is not typed yet, so this pass does not claim that either conversion flag has been selected or applied.
