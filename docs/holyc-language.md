...
function-pointer-declarator := "(" pointer-star{1,4} identifier? ")"
                               "(" parameter-list? ")"
parameter-default  := "=" default-expression
default-expression := literal | identifier | "$$" | "(" default-expression ")"
                    | prefix-operator default-expression
                    | default-expression binary-operator default-expression
prefix-operator    := "+" | "-" | "!" | "~" | "*" | "&" | "++" | "--"
...
Empty and comment-only inputs are valid modules. Declarations retain source order, the exact modifier, binding, type, and identifier spellings, and ordered source segments. A comma-separated group has one shared modifier list, optional binding, and primitive type, plus a source-linked declarator for every name. Each declaration prefix is a separate ordered AST node. An ordinary `extern` or `import` token becomes a binding node without a target. `_extern` and `_import` store the following target identifier in that node, separate from the HolyC name being declared. Each declarator records its terminating comma or semicolon explicitly. Each pointer star is a separate layer numbered from one through four, and numbering starts again for every declarator. A modifier, binding, alternate target, layer, or delimiter keeps its raw spelling, generated frame, invocation span, and definition span. Includes and definitions run through the integrated stream before parsing. A parser diagnostic produced inside an include retains its include stack, while a diagnostic on definition-generated text records both the expansion and declaration sites.
...
The corresponding `Function_prototype` AST node requires one of the four supported bindings and a semicolon. It keeps the ordered prefix nodes, return type and pointer layers separate from the function name, then records both parentheses, every primitive parameter, its register qualifiers and pointer layers, optional parameter names and defaults, commas, a distinct terminal variadic marker, and the semicolon. `extern U8 *CmdLinePmt();` and `public _extern _SYS_HLT U0 SysHlt();` are parameterless pinned examples. `public _extern _CALL I64 Call(U8 *machine_code);` supplies a named pointer parameter. Accepted prototypes enter the streaming symbol environment as functions only after the complete semicolon is present.
...
The default-expression parser accepts integer, floating, character, multi-character, and string literals; identifiers; `$$`; grouped expressions; the eight audited prefix operators; and every binary operator in the pinned table below. It uses the table's numeric precedence and association rather than C precedence. `IC_POWER` receives the special handling from `PrsExpression2`, so ``-2`2`` is represented as ``-(2`2)``. Each term and operator keeps its source span, generated source, and definition origin. A fixed limit of 256 nested expression parses rejects adversarial input with `HCPARSE0021`.

This slice parses but does not execute default expressions. It does not apply type conversions, copy string defaults, set member flags, fill omitted call arguments, or accept the separate `lastclass` default path. Calls, indexing, member access, postfix operators, casts, `sizeof`, and `offset` are outside this core expression slice and receive `HCPARSE0020` when used in a parameter default. Requested parameter registers, array parameters, and unbound function bodies also remain unavailable. `_intern`, global initializers, parenthesized function-pointer globals, function-pointer members and locals, array suffixes, and other global declarator forms are not stored as raw tokens. General unsupported global syntax uses `HCPARSE0001` through `HCPARSE0003`; a fifth star uses `HCPARSE0004`, a parenthesized function-pointer global uses `HCPARSE0005`, malformed function-pointer parameter punctuation uses `HCPARSE0014`, and a missing alternate target uses `HCPARSE0007`. A modifier or binding without a supported primitive type receives `HCPARSE0001` at the following token. Recovery advances to a semicolon or EOF, and the public result contains no AST after any error.
...
The implemented language specification supplies immutable primitive type facts, operator tables, compiler-option facts, and syntax for primitive globals and bound prototypes. It covers ordered declaration prefixes, ordinary and alternate bindings, pointers through depth four, parameter qualifiers, recursive function-pointer parameters, varargs, and core parameter default expressions. It enforces the source parser's AOT gate for both import spellings. It does not resolve bindings, apply modifier linkage or storage effects, build semantic pointer or function types, evaluate defaults, fill call arguments, or support arrays, initializers, function bodies, general expressions, statements, classes, unions, promotions, conversions, aggregate layout, floating execution, compiler-option effects, or semantic diagnostics. Those claims remain absent from the compatibility report until their own source-grounded tests pass.
