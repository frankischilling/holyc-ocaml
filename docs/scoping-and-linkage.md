...
The AST keeps `public` and `static` as ordered staged modifiers. It keeps one ordinary or alternate-name binding separate from the declared type. An `_extern` or `_import` node also retains the target identifier that appears before the type. That separation follows `PrsStmt`, which consumes staged modifiers before it selects a declaration mode.
...
`_intern` remains unavailable because its target is a compile-time expression. Unbound function definitions, function flags, default arguments, class declarations, arrays, initializers, local scope, and duplicate-declaration rules also remain unavailable. [Issue #57](https://github.com/frankischilling/holyc-ocaml/issues/57) records ordinary bindings, [issue #59](https://github.com/frankischilling/holyc-ocaml/issues/59) records alternate-name bindings, and [issue #61](https://github.com/frankischilling/holyc-ocaml/issues/61) records the first function prototype slice. The declaration, symbol, and BIN milestones track the remaining work.

## Reproducible checks

`test/test_parser.ml` checks the exact keyword, mode, staging, function-argument, and variadic definitions; target and punctuation provenance; the AOT gate; streaming symbol visibility; and rejected syntax. `test/cli/parse-bindings.hc`, `test/cli/parse-alternate-bindings.hc`, and `test/cli/parse-function-prototypes.hc` exercise the public CLI in AOT mode. The JIT import fixtures and the function-prototype error fixture record diagnostics and exit status.
