...
`_intern` remains unavailable because its target is a compile-time expression. Unbound function definitions, function-flag validation and ABI effects, default arguments, register allocation, class declarations, arrays, initializers, local scope, and duplicate-declaration rules also remain unavailable. [Issue #57](https://github.com/frankischilling/holyc-ocaml/issues/57) records ordinary bindings, [issue #59](https://github.com/frankischilling/holyc-ocaml/issues/59) records alternate-name bindings, [issue #61](https://github.com/frankischilling/holyc-ocaml/issues/61) records the first function prototype slice, [issue #63](https://github.com/frankischilling/holyc-ocaml/issues/63) records calling modifier syntax, and [issue #65](https://github.com/frankischilling/holyc-ocaml/issues/65) records parameter register qualifiers. The declaration, symbol, ABI, and BIN milestones track the remaining work.

## Reproducible checks

`test/test_parser.ml` checks the exact keyword, mode, staging, function-argument, calling-modifier, register-definition, and variadic behavior; target and punctuation provenance; the AOT gate; streaming symbol visibility; and rejected syntax. `test/cli/parse-bindings.hc`, `test/cli/parse-alternate-bindings.hc`, `test/cli/parse-function-prototypes.hc`, `test/cli/parse-function-modifiers.hc`, and `test/cli/parse-register-parameters.hc` exercise the public CLI. The JIT import fixtures and function error fixtures record diagnostics and exit status.
