...
A 500-case QCheck property supplies arbitrary 64-bit words and selects each of the three unary operations. It executes the original and folded verified graphs through a small test-local evaluator, compares their returned 64-bit words, and checks both results against the direct `Int64.lognot`, zero-truth, or `Int64.neg` formula. This is interpreter-equivalence evidence for the accepted unary word operations only. It does not model general control flow or TempleOS runtime state, and it does not substitute for a production IR interpreter.
...
The compiler driver does not schedule this pass. There is no `holyc` optimizer command, compiler option, optimization-level control, or production interpreter that invokes it. Callers must explicitly provide an `Ir.X87_stack.t` to the library function. Integration with a pass pipeline, compiler controls, interpretation, and backend emission remains unavailable.
