...
The index is immutable and lookup is pure. TempleOS increments `CMemberLst.use_cnt` in `MemberFind`; later use accounting can consume the retained lookup results without changing this pass. Function-body and executable top-level expression typing now use this same index for direct and pointer object paths. Aggregate offset paths in both result owners also use it, but keep an ordered list of lookups and cumulative offsets instead of describing runtime storage. Object-member results retain the selected member, declaring aggregate, inheritance depth, byte layout, type, array rank, callback shape, and source-visible backing class. Offset results retain those lookup facts for every segment, the intermediate member type, and the final `I64` value. Explicit `offset(...)`, metadata lookup, use counting, runtime address calculation, and subinteger access are not part of this index.
...
declarations. Named JIT headers and primitive local allocation positions are
captured. Runtime-sized or aggregate-valued frames and unnamed callback writes
remain guarded. The callback-free
