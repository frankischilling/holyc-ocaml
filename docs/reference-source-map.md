...
In `Compiler/PrsVar.HC:PrsVarLst`, the `PRS1B_FUN_ARG` branch checks for `=` on each member, compiles the following expression with `LexExpression2Bin`, executes the temporary code, converts its result to the parameter type, and sets `MLF_DFT_AVAILABLE`. String results are copied and marked with `MLF_STR_DFT_AVAILABLE`. The branch has no rule requiring defaults to trail, and `lastclass` follows a separate path. `Compiler/PrsExp.HC:PrsExpression2` supplies the precedence loop and its special unary-minus handling for `IC_POWER`; `PrsUnaryTerm` supplies the supported primary and prefix forms. The binary metadata comes from `Compiler/CInit.HC`, with precedence constants from `Compiler/CompilerA.HH`.

`Frontend.Parser` uses that checked binary table directly for parameter defaults, array dimensions, call arguments, indexes, member chains, postfix updates, primitive postfix casts, `sizeof`, `offset`, and `defined` terms, and `_intern` targets. Only entries marked `ASSOCF_RIGHT` group to the right; all remaining entries group to the left, as the source loop does. The parser represents an absent default separately from an explicit expression and applies the same grammar inside function-pointer signatures. Every term and operator retains source and definition provenance. This stage does not execute expressions, perform the source conversions, set member or internal-function flags, store strings, fill omitted call arguments, validate index, member, update, cast, `sizeof`, or `offset` operands, resolve the truth of a `defined` term, or assign executable addresses. Resolved nonprimitive cast targets and special forms receive `HCPARSE0020`, while a fixed 256-level depth guard reports `HCPARSE0021`.

`Compiler/PrsExp.HC:PrsFunCall` accepts an opening parenthesis when present and walks the fixed member list in declaration order. For a member carrying `MLF_DFT_AVAILABLE`, a current `)`, a current `,`, or a call made without parentheses selects the stored default. `Doc/HolyC.DD` documents both parentheses-free default-only calls and the non-trailing default example `Test(,3);`. The pinned compiler itself uses consecutive holes in `PrsStmt(cc,,,cmp_flags)`, leading holes in `GetStr(,,GSF_SHIFT_ESC_EXIT)`, and a middle hole while loading `OpCodes.DD`.
