...
After those modifiers, `PrsStmt` recognizes ordinary `KW_EXTERN` and `KW_IMPORT`, whose keyword IDs are 10 and 27. They select the `PRS0_EXTERN` and `PRS0_IMPORT` declaration modes defined as `0x000004` and `0x000005` in `Compiler/CompilerA.HH`. The underscore paths use `KW__EXTERN` and `KW__IMPORT`, whose IDs are 11 and 28, consume a target identifier before the type, set the `FSF__` staging bit, and select `PRS0__EXTERN` or `PRS0__IMPORT`. `PrsGlblVarLst` rejects both import modes unless `CCF_AOT_COMPILE` is set. `Kernel/KernelC.HH:10` supplies a grouped ordinary example: `public extern U8 *rev_bits_table,*set_bits_table;`. `Kernel/KernelB.HH:37` and `Kernel/KernelC.HH:23-29` supply alternate-name globals, while `Compiler/CExts.HC:42-46` supplies alternate-name arrays. The OCaml parser retains one source-linked binding node after the modifier list, stores an alternate target as a separate identifier, and applies the same AOT gate. It does not yet look up that target, resolve aliases, or apply `OPTf_EXTERNS_TO_IMPORTS`. `_intern` remains separate because the pinned path obtains its target from `LexExpressionI64`.
