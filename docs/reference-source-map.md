...
`Frontend.Parser` accepts `modifier* binding? primitive declarator (, declarator)* ;`, where a modifier is `public`, `static`, or one of the four calling modifiers, a binding is ordinary `extern` or `import` or alternate `_extern target` or `_import target`, and each declarator has zero through four pointer stars and one identifier. The primitive is one of the 12 public spellings checked by `Sema.Primitive_type`. The parser reuses the modifier list, binding, and base type across the declaration and starts pointer depth at zero for each name, matching the saved inputs in `PrsGlblVarLst`. Pinned examples include `Demo/MultiCore/Primes.HC:2`, `Demo/MultiCore/MPRadix.HC:11`, `Kernel/KDefine.HC:16`, `Kernel/KernelB.HH:37`, and `Kernel/KernelC.HH:10`. `Kernel/KernelC.HH:17` supplies the bound `public argpop extern` order. Type names remain ordinary identifier tokens, matching the compiler hash-table design rather than introducing lexer-only type keywords. Parenthesized function-pointer globals follow a separate `PrsType` path and remain unsupported, as do `_intern`, array suffixes, class and union prefixes, and anonymous forms.
