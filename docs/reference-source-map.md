...
After those modifiers, `PrsStmt` recognizes ordinary `KW_EXTERN` and `KW_IMPORT`, whose keyword IDs are 10 and 27. They select the `PRS0_EXTERN` and `PRS0_IMPORT` declaration modes defined as `0x000004` and `0x000005` in `Compiler/CompilerA.HH`. `PrsGlblVarLst` rejects `PRS0_IMPORT` unless `CCF_AOT_COMPILE` is set. `Kernel/KernelC.HH:10` supplies a grouped source example: `public extern U8 *rev_bits_table,*set_bits_table;`. The OCaml parser retains one source-linked binding node after the modifier list and applies the same AOT gate. Binding resolution, `OPTf_EXTERNS_TO_IMPORTS`, and the underscored `_extern`, `_import`, and `_intern` paths remain separate work.
