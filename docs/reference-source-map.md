...
Before type dispatch, `PrsStmt` recognizes `KW_PUBLIC` and `KW_STATIC`. `Compiler/CompilerA.HH` assigns them keyword IDs 25 and 41 and assigns `FSF_PUBLIC` and `FSF_STATIC` to staging bits `0x01` and `0x04`. The `public` case retains calling, public, and assembly state; the `static` case retains assembly state while clearing the other staged bits. `PrsGlblVarLst` checks the resulting public bit when it publishes a global. `Frontend.Parser` keeps each modifier token as an ordered, source-linked AST node. It does not yet apply export, linkage, storage, or local-static semantics.
