...
Outside `CCF_IN_IF`, the pinned lexer discards an unmatched `#endif`, treats an unmatched `#else` as a request to scan forward, and returns EOF without a mismatch diagnostic when a false branch is unterminated. `Frontend.Preprocessor` keeps `HCPP0015` through `HCPP0018` under its default `hosted-strict` policy. The opt-in `templeos-permissive` policy follows the source scan instead, including nested openers and frame changes. The versioned preprocessing report names the selected policy and reference commit. The source-derived fixtures are executable, but [issue #27](https://github.com/frankischilling/holyc-ocaml/issues/27) remains open until a controlled native run records the same cases. The `KW_IFJIT` path also returns `TK_IFAOT` while `CCF_IN_IF` is set; that source quirk concerns later `#if` expression parsing and is not part of the mode-only state.
