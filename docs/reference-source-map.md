...
`Frontend.Parser` uses that checked binary table directly for parameter defaults, array dimensions, call arguments, indexes, member chains, and `_intern` targets. Only entries marked `ASSOCF_RIGHT` group to the right; all remaining entries group to the left, as the source loop does. The parser represents an absent default separately from an explicit expression and applies the same grammar inside function-pointer signatures. Every term and operator retains source and definition provenance. This stage does not execute expressions, perform the source conversions, set member or internal-function flags, store strings, fill omitted call arguments, validate index or member bases, or assign executable addresses. Postfix increment or decrement, cast, and special forms receive `HCPARSE0020`, while a fixed 256-level depth guard reports `HCPARSE0021`.
