...
`holyc-ir-literal-v1` records the source kind, result type, checked instruction chain, source spans, and pinned reference commit. `lower_typed_result` copies those facts from a payload-bearing semantic leaf, including nested parentheses and unary plus, without consulting a frontend AST node. Emitting semantic prefixes, multi-operand expression graphs, string storage, unary folding, and nonliteral roots remain outside this boundary.
