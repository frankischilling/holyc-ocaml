...
Here, an operand graph means an IR operand and use graph. Semantic parentheses and prefix operators retain one exact checked child. A completed semantic binary result retains its exact checked left and right children in source order, including any conversion-bearing replacement produced while checking an assignment. These child links do not allocate instructions, blocks, uses, or values. The limited numeric expression lowerer consumes these links only for its documented integer, pure `F64`, address, dereference, and binary shapes; all other semantic records remain outside IR.
