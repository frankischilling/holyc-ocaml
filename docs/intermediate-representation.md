...
`Compiler/PrsExp.HC:614-620` selects `IC_COM`, `IC_NOT`, and `IC_UNARY_MINUS`; lines 133-162 add them after the operand. `Compiler/CInit.HC:50-52` fixes all three one-argument, one-result shapes. The lowerer keeps the literal instruction unfused and adds one checked unary instruction per prefix from innermost to outermost. Each uses the previous result as its operand and retains the operator token span. Unary minus and logical not forward the current type, while `OptPass012.HC:153-161` explicitly changes complement results to internal `I64`. Folding remains in `OptPass012.HC:153-192`, outside this boundary. `OptLib.HC:230-242` and `357-369` separately consume logical not when simplifying conditional branches.

Callers provide unary instruction and value identities in that inner-to-outer order. `HCIRL0001` rejects a missing or excess identity before an unchecked sequence can escape. For a nonliteral expression wrapped only in supported grouping, unary-plus, unary-minus, logical-not, and bitwise-complement nodes, `lower_expression` returns `Not_literal` without constructing IR. Dereference and address-of remain outside this boundary. Semantic traversal identities are not reused as IR identities.
