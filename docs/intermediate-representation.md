...
Grouping and unary plus remain transparent. Integer leaves, numeric unary nodes, and binary nodes require a checked, zero-depth, nonzero integer type and an integer result class. Pointer-typed intermediates are accepted only for checked address and dereference nodes; they do not broaden the binary rules. F64, power, assignment and compound assignment, identifiers, calls, pointer arithmetic, updates, conversions, lvalue storage, and short-circuit branches return `Unsupported_expression`. `HCIRL0004` reports inconsistent semantic metadata, including a missing prefix operand, a missing checked pointer result, or a unary result type that does not follow the audited rule. `HCIRL0005` reports instruction or value identity exhaustion. `holyc-ir-expression-v1` records the final value, result type, next IDs, checked sequence, and pinned reference commit.
