...
`Holyc_lib.Ir_literal_lowering` is the first checked instruction-producing boundary. The low-level `lower` entry point accepts one explicit literal payload, stable instruction and value IDs, and an optional span. `lower_expression` accepts a frontend expression and derives the payload and span from integer, character, F64, and string literal nodes, either directly or through nested parentheses and unary-plus prefixes. Both paths construct accepted input through `Ir_instruction_sequence.create`.
...
`Compiler/PrsExp.HC:728-747` compiles a parenthesized term by entering `PrsExpression` for the inner value and adds no instruction for the parentheses. Lines 750-759 likewise consume unary plus without selecting an opcode, while lines 138-162 add a unary instruction only for a nonzero operator entry. The hosted boundary therefore unwraps grouping and unary plus iteratively, preserves the inner literal span, and produces the same single instruction as the direct form. Neither wrapper receives an IR identity.

For a nonliteral expression, including one wrapped in grouping or unary plus, `lower_expression` returns `Not_literal` without allocating an instruction or consuming an identity. Unary minus, logical not, bitwise complement, dereference, and address-of remain nonliteral results at this boundary because they are not transparent in the pinned compiler. The caller still supplies fresh instruction and value IDs because semantic traversal identities are not IR identities.
