...
Grouping and unary plus remain transparent. Each accepted node must have a checked, zero-depth, nonzero integer type and an integer result class. F64, power, assignment and compound assignment, identifiers, calls, pointers, dereference, address-of, updates, conversions, pointer scaling, lvalue behavior, and short-circuit branches return `Unsupported_expression`. `HCIRL0004` reports inconsistent semantic metadata, including a missing prefix operand or a unary result type that does not follow the audited rule. `HCIRL0005` reports instruction or value identity exhaustion. `holyc-ir-expression-v1` records the final value, result type, next IDs, checked sequence, and pinned reference commit.
