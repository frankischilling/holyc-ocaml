...
`holyc-ir-literal-v1` records the source kind, result type, checked instruction chain, source spans, and pinned reference commit. The module consumes parser nodes but does not traverse a typed semantic expression result, combine separate expression operands into a graph, allocate string storage, optimize unary instructions, or lower nonliteral roots.
