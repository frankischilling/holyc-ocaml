...
`Holyc_lib.Ir_literal_lowering` is the first checked instruction-producing boundary. The low-level `lower` entry point accepts one explicit literal payload, stable instruction and value IDs, and an optional span. `lower_expression` accepts a frontend expression and derives the payload and span from integer, character, F64, and string literal nodes through nested parentheses, unary plus, unary minus, and logical not. The literal and every emitting prefix receive caller-owned instruction and value identities. Both paths construct accepted input through `Ir_instruction_sequence.create`.
...
`Compiler/PrsExp.HC:614-620` selects `IC_NOT` and `IC_UNARY_MINUS`; lines 133-162 add them after the operand and forward the operand class. `Compiler/CInit.HC:51-52` fixes both one-argument, one-result shapes. The lowerer keeps the literal instruction unfused and adds one checked unary instruction per prefix from innermost to outermost. Each uses the previous result as its operand, forwards the literal result type, and retains the operator token span. Folding remains in `OptPass012.HC:163-192`, outside this boundary. `OptLib.HC:230-242` and `357-369` separately consume logical not when simplifying conditional branches. The string-pointer path follows the same integer-side instruction shape used by the pinned compiler.

Callers provide unary instruction and value identities in that inner-to-outer order. `HCIRL0001` rejects a missing or excess identity before an unchecked sequence can escape. For a nonliteral expression wrapped only in supported grouping, unary-plus, unary-minus, and logical-not nodes, `lower_expression` returns `Not_literal` without constructing IR. Bitwise complement, dereference, and address-of remain outside this boundary. Semantic traversal identities are not reused as IR identities.
