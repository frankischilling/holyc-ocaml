...
Pure `F64` trees accept `IC_MUL`, `IC_DIV`, `IC_ADD`, and `IC_SUB`. `Compiler/OptLib.HC:96-179` resolves these operations to `FBO1_F64` when both checked operands are floating. `Compiler/OptPass012.HC:308-629` retains or folds the four operations, and `Compiler/OptPass789A.HC:518-565` selects their floating backend when `ICF_USE_INT` is clear. Unary plus remains transparent; unary minus and logical not retain the checked `F64` type. Literal bits pass through unchanged, and division by a literal zero remains an ordinary `IC_DIV` because this boundary does not evaluate it.

Mixed integer/`F64` trees accept the same four arithmetic operations. `Kernel/KernelA.HH:1602` defines `ICF_RES_TO_F64`, and `OptFixupBinaryOp1` applies that intent to a non-floating producer when the combined class is `F64`. The checked lowerer places the bit on the final instruction of each integer operand subtree. Parentheses and unary plus pass the request to the retained producer. Earlier nodes in the integer subtree keep zero flags. The canonical pre-optimizer form also leaves an integer literal as `IC_IMM_I64` with its exact integer payload and the conversion bit; `OptFixupBinaryOp1` later rewrites such an immediate to `IC_IMM_F64`. The lowerer does not perform that numerical conversion.

Integer leaves, integer unary nodes, and integer binary nodes require a checked, zero-depth, nonzero integer type and an integer result class. Floating leaves and accepted floating operations require internal `F64` and an `F64` result class. Pointer-typed intermediates are accepted only for checked address and dereference nodes; they do not broaden the binary rule. Power, floating comparisons, modulo, assignment and compound assignment, identifiers, calls, pointer arithmetic, updates, result-to-integer conversions, lvalue storage, and short-circuit branches return `Unsupported_expression`. `HCIRL0004` reports inconsistent semantic metadata, including a missing prefix operand, a missing checked pointer result, or a unary result type that does not follow the audited rule. `HCIRL0005` reports instruction or value identity exhaustion. `holyc-ir-expression-v1` records the final value, result type, next IDs, checked sequence, flags, and pinned reference commit.
