...
`Compiler/PrsExp.HC:614-620` selects `IC_COM`, `IC_NOT`, `IC_UNARY_MINUS`, and `IC_DEREF`; lines 133-162 add them after the operand. `Compiler/CInit.HC:50-53` fixes all four one-argument, one-result shapes. The lowerer keeps the literal instruction unfused and adds one checked unary instruction per prefix from innermost to outermost. Each uses the previous result as its operand and retains the operator token span. Unary minus and logical not forward the current type, while `OptPass012.HC:153-161` explicitly changes complement results to internal `I64`. Dereference removes one pointer class when the operand has one and otherwise keeps the operand class, matching `PrsExp.HC:149-156`. Folding remains in `OptPass012.HC:153-192`, outside this boundary. `OptLib.HC:230-242` and `357-369` separately consume logical not when simplifying conditional branches.

Callers provide unary instruction and value identities in that inner-to-outer order. `HCIRL0001` rejects a missing or excess identity before an unchecked sequence can escape. For a nonliteral expression wrapped only in supported grouping, unary-plus, unary-minus, logical-not, bitwise-complement, and dereference nodes, `lower_expression` returns `Not_literal` without constructing IR. Address-of remains outside this boundary. Semantic traversal identities are not reused as IR identities.
