...
Integer leaves, integer unary nodes, and integer binary nodes require a checked, zero-depth, nonzero integer type and an integer result class. Pure floating leaves and accepted floating operations require internal `F64` and an `F64` result class. Pointer-typed intermediates are accepted only for checked address and dereference nodes; they do not broaden either binary rule. Mixed integer and `F64` operands, power, floating comparisons, modulo, assignment and compound assignment, identifiers, calls, pointer arithmetic, updates, conversions, lvalue storage, and short-circuit branches return `Unsupported_expression`. Mixed operands stay outside this slice because `OptFixupBinaryOp1` records explicit `ICF_RES_TO_F64` intent that the checked sequence does not yet represent. `HCIRL0004` reports inconsistent semantic metadata, including a missing prefix operand, a missing checked pointer result, or a unary result type that does not follow the audited rule. `HCIRL0005` reports instruction or value identity exhaustion. `holyc-ir-expression-v1` records the final value, result type, next IDs, checked sequence, and pinned reference commit.
