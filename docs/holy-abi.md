...
A direct `&Function` expression now retains the exact source publication and canonical function identity selected by source order. Its semantic type is TempleOS's internal `RT_PTR`, an intrinsic `RT_I64` value with no additional object-pointer layer. This is input to later call and relocation work, not an executable address. The JIT extern-slot load, JIT definition immediate, AOT absolute address, import record, relocation width, and eventual argument placement remain unimplemented. [Issue #286](https://github.com/frankischilling/holyc-ocaml/issues/286) records this ABI input boundary.
