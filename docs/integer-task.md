...
the driver installs missing checked StreamExePrint/StreamPrint/Print/PutChars
headers once. Their real setup work is charged. Comments, strings and skipped
directives do not activate a task. Generated text returns to the ordinary outer
parser, including joined tokens, expressions, function bodies and initializer
operands. The final outer unit executes in a fresh isolated image with no task
namespace access.
...
captures `AB` under `holyc run --mode=aot`. It uses 40 runtime instructions,
...
Each provider declaration is a source command. The four-provider setup charges
four command units. Adding StreamExePrint therefore adds one task instruction to
source paths that activate the hosted providers. Sources that never enter `#exe`
retain their previous accounting.
...
The maintained adapter fixture uses 23 task instructions including provider
...
## StreamPrint and StreamExePrint services
