...
`Kernel/KernelA.HH:3723` and `:3728` also use `#define` followed directly by a string as section headings. In the pinned `KW_DEFINE` branch, the lexer reads that string while definition expansion is disabled, finds no identifier, and resumes after it without entering replacement capture. The hosted stream follows that control flow: the string disappears, no definition or help record is created, and a token later on the same line remains available. Other invalid definition-name tokens still report `HCPP0010` until their source behavior is audited separately.
