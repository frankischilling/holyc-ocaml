...
Function-local direct assembly has a separate AST node for adjacent opcodes and aliases with zero, one, or two operands. Each operation keeps its register, immediate, or memory operands, size and segment prefixes, comma, optional semicolon, source spelling, canonical opcode identity, and generated-source provenance. This follows the pinned `CMPF_ONE_ASM_INS` and `PrsAsmArg` paths without treating physical newlines as instruction boundaries. `HCPARSE0147` rejects direct opcodes at top level, while `HCPARSE0149` through `HCPARSE0152` report malformed operand structure. Direct assembly directives and labels, operand validation, form selection, lowering, encoding, and execution remain unavailable.
