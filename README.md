...
The `lex` and `preprocess` commands exit with status 1 when they report a diagnostic. Both treat offsets and columns as byte positions, matching the byte-oriented pinned source. `preprocess` currently accepts `#include` and reports every other directive as unsupported; its path rules and hosted limits are documented in [docs/preprocessor.md](docs/preprocessor.md).
