...
Execution beyond the bounded integer program domain, optimization beyond the verified integer-unary pass, x86-64 emission, general hosted runtime services, native JIT execution, the assembler encoder, and the TempleOS `.BIN` writer remain unfinished. The opcode and BIN APIs are audited specifications, not encoders or loaders. Unsupported parser input reports an `HCPARSE` diagnostic and prevents a successful public AST; there is no raw-token fallback.
