...
The parser accepts primitive globals, pointer and array declarators, comma-separated declaration groups, declaration and calling modifiers, ordinary and alternate-name bindings, `_intern` expression targets, bound primitive function prototypes, and unbound primitive function definitions. Function signatures include named and unnamed parameters, recursive function-pointer parameters, register qualifiers, terminal varargs, ordinary defaults in non-trailing positions, and a distinct `lastclass` default. Function bodies accept primitive automatic and static locals. Local groups retain pointer layers, array dimensions, scalar initializers, automatic `reg` and `noreg` requests, and an optional canonical U64 register after `reg`. An explicit `;` remains an empty body statement, while a signature that reaches EOF has an absent body.
...
The current parser work is syntax only. Its function-wide lookup context routes identifiers and preprocessor conditionals, but it does not create semantic scopes, resolve types or bindings, allocate local storage, calculate class layouts, validate calls, substitute defaults, resolve `Print` or `PutChars`, bind `goto` targets, reject duplicate or unresolved labels, propagate `lock` regions to `ICF_LOCK`, assign implicit case values, validate switch labels, enforce sub-switch front-porch rules, build switch tables, validate a `break` target or return context, check a return value against a function type, unwind active `try` regions, call `SysTry` or `SysUntry`, convert a condition to truth, build control flow, lower expressions, or execute top-level statements. Class and union locals, local function-pointer declarators, aggregate initializers, and many remaining corpus declaration forms still fail with explicit diagnostics. Classes, unions, exception semantics, lock-aware IR and code emission, assembly syntax, and whole-corpus parsing also remain unavailable.
