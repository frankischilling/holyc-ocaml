...
The AOT parser corpus baseline measures the same 528 committed objects in fresh compiler sessions. Twenty currently parse without an error; 16 have a lexer or preprocessor first error and 492 have a parser first error. There are no read errors, internal errors, or omitted files. This is a progress measurement for syntax coverage, not a semantic compatibility score. The full per-file result is checked in at [`reference/parser-corpus-aot.json`](reference/parser-corpus-aot.json).
