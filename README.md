...
`holyc run --target=ir examples/integer-arrays.hc` executes the documented array program through semantic checking, verified IR and the bounded interpreter. The earlier integer function, control-flow, global, static and pointer examples use the same path. `eval-native` separately compiles and executes the supported integer-expression subset, while `run --target=host-jit` adds structured control flow, fixed direct scalar integer functions, width-correct automatic locals and U0 procedures. General native declarations, persistent and pointer storage, the full ABI and complete HolyC execution remain unfinished.
