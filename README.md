...
The current parser work is syntax only. It does not create lexical scopes for blocks or functions, resolve types or bindings, calculate storage or class layouts, validate calls, substitute defaults, resolve `Print` or `PutChars`, bind `goto` targets, reject duplicate or unresolved labels, propagate `lock` regions to `ICF_LOCK`, assign implicit case values, validate switch labels, enforce sub-switch front-porch rules, build switch tables, validate a `break` target or return context, check a return value against a function type, unwind active `try` regions, call `SysTry` or `SysUntry`, convert a condition to truth, build control flow, lower expressions, or execute top-level statements. Function definitions cannot yet contain local declarations, so most corpus functions still fail to parse. Classes, unions, exception semantics, lock-aware IR and code emission, assembly syntax, and whole-corpus parsing also remain unavailable.
