...
The `lex` and `preprocess` commands exit with status 1 when they report a diagnostic. Both treat offsets and columns as byte positions, matching the byte-oriented pinned source. `preprocess` currently accepts `#include`, `#define`, `#ifjit`, `#ifaot`, `#else`, and `#endif`. JIT is the default; `--mode=aot` selects AOT branches. Definition rules, conditional behavior, path handling, provenance, and hosted limits are documented in [docs/preprocessor.md](docs/preprocessor.md).
