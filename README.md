...
Ordinary expression statements inside checked function definitions use the same immutable expression result as call arguments. Each statement keeps its recursive source order, source origin, checked type, result class, nested call identity, and explicit `ICF_RES_NOT_USED` intent. Accepted numeric function and top-level statements emit `IC_END_EXP`; a target-aware top-level path also terminates standalone checked direct calls after their complete call fragment. The integer program path executes discarded values, supported nested calls and scalar side effects. The bounded interpreter executes implicit Print and PutChars; the native target also executes the checked PutChars provider.
