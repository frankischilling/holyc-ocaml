...
Top-level string and character statements now have explicit AST nodes. Strings select `Print`; characters select `PutChars`. Empty markers such as `"" fmt,args;` and `'' value;` retain the following fixed expression, while a nonempty marker remains the start of its complete expression. Additional `Print` arguments, punctuation, source order, and generated-source provenance remain visible in AST dumps.
