...
The declaration and statement parser uses complete reads of `Compiler/Compiler.PRJ`, `Compiler/CompilerA.HH`, `Compiler/CompilerB.HH`, `Compiler/CExts.HC`, `Compiler/CExcept.HC`, `Compiler/PrsLib.HC`, `Compiler/PrsVar.HC`, `Compiler/PrsStmt.HC`, `Compiler/CMain.HC`, `Compiler/CInit.HC`, `Kernel/KernelB.HH`, `Kernel/KernelC.HH`, and `Doc/HolyC.DD`. The array audit also checks expression and initialized declarations in `Compiler/Lex.HC` and `Compiler/Asm.HC`, plus pointer and multidimensional forms in `Adam/Gr/GrGlbls.HC`. The `for` audit checks empty initializers in `Kernel/StrPrint.HC` and empty or comma-linked updates in `Adam/Opt/Utils/StrUtils.HC`. The `break` audit follows its dispatcher, target check, jump, and common comma-or-semicolon boundary in `PrsStmt`, with switch examples in `Doc/HolyC.DD`. The `lock` audit follows its nesting counter in `PrsStmt`, the `ICF_LOCK` handoff in `PrsLib`, and both braced and unbraced examples in `Demo/MultiCore/LoadTest.HC` and `Demo/MultiCore/Lock.HC`. `Doc/Lex.DD`, `Doc/Options.DD`, and `Doc/ScopingLinkage.DD` provide supporting language context. The manifest records every checksum.
