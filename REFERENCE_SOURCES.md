...
Implicit output target resolution follows the `Print` and `PutChars` branches in `Compiler/PrsExp.HC`, the ordinary prototypes in `Kernel/KExts.HC` and `Kernel/KernelC.HH`, and the type-masked bucket and table-chain search in `Kernel/KHashA.HC`. All four files already have pinned checksums in the manifest. `Sema.Implicit_output_target_resolution` retains a visible module header and canonical identity or the exact outer-table entry. It does not copy a runtime signature into an outer record, apply an argument conversion, format text, lower a call, or execute output.

Implicit output argument binding follows the fixed-slot loop, default branch, conversion flags, and `Print` variadic tail at `Compiler/PrsExp.HC:430-511`. `Sema.Implicit_output_argument_binding` consumes the selected checked header, preserves whether each fixed slot was provided or defaulted, and retains the extra values only for a variadic header. A JIT default stays immediate; an AOT string-bearing default records the source-required string-constant path. An outer target remains deferred unless the caller supplies a checked header for the exact resolved symbol. The implementation copies no TempleOS code and does not parse formats, evaluate default payloads, lower calls, or execute output.
...
Local member-list classification follows `Compiler/PrsVar.HC:490-524`, where every new local starts with zero flags, static mode adds `MLF_STATIC`, and a returned callback adds `MLF_FUN`; `Compiler/PrsExp.HC:763-804`, where those bits select callback metadata and static rather than frame-relative address handling; the mask definitions in `Kernel/KernelA.HH`; and the static-function-variable note in `Doc/ScopingLinkage.DD`. `Sema.Local_type_resolution` derives the declaration-time mask from its checked storage and declarator shape, without copying source code or accepting a caller-supplied value. Pinned tests retain declarations from `Kernel/KMisc.HC`, `Adam/WinMgr.HC`, and `Adam/Gr/SpriteMesh.HC`. Static allocation, initializer evaluation, address lowering, later warning-bit mutations, and execution remain outside this boundary.
...
## Member callback call audit

Member callback resolution follows the exact pinned path at `Compiler/PrsExp.HC:998-1045`. `MemberFind` selects the current or inherited `CMemberLst`; `MLF_FUN` distinguishes callback storage from an ordinary field; `PrsPopDeref` preserves the function value; and the following `(` enters indirect `PrsFunCall`. The argument and return behavior comes from `Compiler/PrsExp.HC:412-578`. Duplicate behavior is grounded in `Compiler/LexLib.HC:67-118`: lookup visits the current class before its base, while `MemberAdd` rejects ordinary direct or inherited duplicate names.

`Sema.Aggregate_member_index` retains the exact callback return reference and recursive header beside each completed member. `Sema.Function_call_resolution` uses that data for direct and pointer member calls, preserving the computed object expression and lookup rather than reconstructing a signature from the field size. Focused tests cover both compilation modes, defaults, holes, varargs, inherited lookup, primitive, pointer, and aggregate returns, parenthesized callees, definition provenance, invalid member shapes, foreign indexes, and deterministic replay. The compiler does not read callback storage, calculate its address, lower the call, or execute it. No TempleOS code or table is copied.

## `no_warn` statement audit

The statement audit follows `Compiler/PrsStmt.HC:PrsNoWarn`, its dispatch and common comma/semicolon loop, local-first lookup in `Compiler/Lex.HC`, `MemberFind` in `Compiler/LexLib.HC`, `CMemberLst` and `MLF_NO_UNUSED_WARN` in `Kernel/KernelA.HH`, and the descriptions in `Doc/HolyC.DD` and `Doc/Options.DD`. The live fixtures come from `Apps/Budget/BgtMain.HC`, `Kernel/SerialDev/Mouse.HC`, `Kernel/KTask.HC`, `Kernel/KDbg.HC`, `Demo/MultiCore/Palindrome.HC`, and `Demo/MultiCore/MPRadix.HC`. `Adam/Ctrls/CtrlsSlider.HC` supplies the separate occurrence inside generated HolyC text. The manifest now checks every one of those Git blobs individually.

`Frontend.Parser` and `Frontend.Ast` retain the observed target and separator shape without copying TempleOS code. Lookup uses the existing function-local parser environment and does not mutate the semantic `CMemberLst` record. Flag application, use accounting, compiler-option state, and warning emission remain separate work.
