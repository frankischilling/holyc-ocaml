...
let execute_prepared ?(callees = [||]) ?(aot_linked = false)
  ...
  let invoke_output block instruction site parameter_types scope =
    ...
    let provider_message message =
      let message =
        if
          (provider_name = "StreamPrint" || provider_name = "StreamExePrint")
          && String.starts_with ~prefix:"Print " message
        then provider_name ^ String.sub message 5 (String.length message - 5)
        else message
      in
      if
    ...
    then
      ...
      let selected_output =
        ...
        | Some Runtime.Stream_exe_print ->
            let max_output_bytes =
              Option.fold
                ~none:(16 * 1024 * 1024)
                ~some:Output.capacity generation_output
            in
            Option.map
              (fun output -> Output.share_work output ~max_output_bytes)
              output
        | _ -> output
      in
      ...
      | Some output -> (
          ...
          | Some Runtime.Put_chars, [ Runtime_word word ] ->
              ...
              |> Result.map (fun () -> Completed_void)
          | ( Some
                (Runtime.Print | Runtime.Stream_print | Runtime.Stream_exe_print),
            ...
            when count.type_ = I64
              ...
              if inactive then
                ...
                | Some execute ->
                    ...
                    let nested, nested_steps =
                      execute ~steps:!steps ~frame_bytes ~call_depth formatted
                    in
                    if nested_steps < 0 || nested_steps > max_steps - !steps
                    then
                      error "HCIRVM0007"
                        "nested source exceeded the remaining execution step \
                         limit"
                    else (
                      steps := !steps + nested_steps;
                      match nested with
                      | Ok bits -> Ok (Completed_word { type_ = I64; bits })
                      | Error [] ->
                          error "HCIRVM0026" "nested source execution failed"
...
let execute_program_with_output ?task ?isolated_budget
  ...
  in
  let accounted_steps = ref 0 in
  let nested_stream_exe_print =
    match (task, stream_exe_print) with
    | Some task, Some execute ->
        Some
          (fun ~steps ~frame_bytes ~call_depth source ->
...
let execute_task_program ?(use_active_stream = true) ?stream_exe_print task
  ...
  let result =
    ...
    else if available_frame_bytes <= 0 then
      Error
        [
          make_error ~stage:Preflight ~executed_steps:0 "HCIRVM0011"
            "nested source has no remaining active frame byte allowance";
        ]
    else if available_call_depth <= 0 then
      Error
        [
          make_error ~stage:Preflight ~executed_steps:0 "HCIRVM0015"
