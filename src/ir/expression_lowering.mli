...
(** Lower an integer, character, [F64], ordinary current-position, completed
    ...
    addition, subtraction, the six comparisons, and the three logical binary
    operations. A mixed edge marks the retained integer producer with
    [ICF_RES_TO_F64] without folding its payload. Floating comparisons also
    carry [ICF_USE_F64], while floating logical operations remain unflagged;
    both retain the checked internal [I64] result. A completed aggregate offset
    emits an internal [I64] [IC_IMM_I64] from its retained final cumulative byte
