...
(** Parse ordinary source without command or [#exe] executors. Compile the exact
    ...
    with width-correct automatic/global integer storage and function-local
    language goto/labels, through the shared native word backend. Supported
    scalar parameter defaults are prepared once at their original declaration
    callbacks through the checked constant-preparation engine. Ordinary scalar
    globals without declaration initializers retain their exact source
    declarations and checked address metadata. Global initializers, statics,
