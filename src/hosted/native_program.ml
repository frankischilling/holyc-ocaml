...
let compile_with_preparation ?(max_ir_instructions = 4096)
    ...
    ~preparation_steps ~default_bytes session ~config ~source =
...
let compile ?max_ir_instructions ?max_code_bytes ?max_stack_bytes ?max_blocks
    ?max_initializer_steps ?max_default_bytes ?max_global_bytes ?status_abi
    session ~config ~source =
  compile_with_preparation ?max_ir_instructions ?max_code_bytes ?max_stack_bytes
    ?max_blocks ?max_initializer_steps ?max_default_bytes ?max_global_bytes
    ?status_abi ~preparation_steps:(ref 0) ~default_bytes:(ref 0) session
    ~config ~source
  ...
  then
    ...
    match
      compile_with_preparation ?max_ir_instructions ?max_code_bytes
        ...
        ~max_global_bytes ?status_abi ~preparation_steps ~default_bytes session
        ~config ~source
    with
    | Error diagnostics ->
        {
