...
let resolve_completed_function_header ?selected_aggregate session ~namespace
    declaration =
  Driver.Function_type_resolution.resolve_completed_header ?selected_aggregate
    ~table:(Session.semantic_symbols session)
...
let layout_global_arrays session ~bindings module_ =
  Driver.Global_array_layout.layout
    ~table:(Session.semantic_symbols session)
    ~bindings module_
...
let type_function_call_expressions_with_outer session ~members ~outer ~policies
    =
  Sema.Function_call_expression_result.analyze
    ~table:(Session.semantic_symbols session)
    ~members ~outer policies
...
let collect_top_level_switch_selectors session expressions =
  Sema.Top_level_switch_selector_result.collect
    ~table:(Session.semantic_symbols session)
    expressions

let collect_top_level_switch_cases session expressions =
  Sema.Top_level_switch_case_result.collect
    ~table:(Session.semantic_symbols session)
    expressions
...
let resolve_top_level_implicit_output_targets session ~function_types ~functions
    expressions =
