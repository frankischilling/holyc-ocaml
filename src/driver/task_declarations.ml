      ...
      protect (fun () ->
          ...
          List.iteri
            (fun item_index -> function
                      ...
                      | Global { publication; completed = Some completed }
                        when completed == declarator ->
                          ...
                          add item_index ~declarator_index
                            Collection.Global_variable declarator.name assigned
                      | _ ->
                          fail declarator.location.span
                            "global command substituted or lacks its completed \
                             declarator")
                    declaration.declarators
              | Ast.Function_prototype prototype -> (
                  let assigned = find ledger prototype.name in
                  match assigned.source with
                  | Function { publication; header = Some header; body = None }
                    ->
                      if
                        (not
                           (same_option ( == )
              ...
              | Ast.Aggregate_forward_declaration _ | Ast.Aggregate_definition _
                ->
