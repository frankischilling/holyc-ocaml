...
let compile ?(max_ir_instructions = 4096) ?(max_code_bytes = 65536) session
    ~config ~source =
  ...
  Image.compile ~max_ir_instructions ~max_code_bytes graph
  |> Result.map_error errors

let evaluate ?max_ir_instructions ?max_code_bytes session ~config ~source =
  let* image =
    compile ?max_ir_instructions ?max_code_bytes session ~config ~source
  in
