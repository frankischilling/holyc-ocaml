...
type conditional_recovery =
  | Hosted_strict
  | Templeos_permissive
      (** [Hosted_strict] diagnoses unmatched boundaries. [Templeos_permissive]
          follows the pinned lexer's silent conditional scan and EOF recovery.
          Hosted resource and input limits apply to both policies. *)
...
(** Versioned reports name the selected recovery policy. [reference_commit]
    identifies the pinned TempleOS source used for the comparison. *)
