...
val classify :
  ...
  Function_call_expression_result.top_level_direct_call ->
  (t, error) result
(** Join a typed top-level direct call to the exact declaration-order function
    record retained for its source position. *)

val source : t -> Function_call_expression_result.top_level_direct_call
